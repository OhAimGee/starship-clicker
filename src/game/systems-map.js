...
// PRNG minimal (mulberry32), seedé par l'index du système — suffisant pour
// varier la composition sans dépendre d'un état externe.
...
/** Butin ponctuel d'une planète `invaded` conquise. */
export function planetLoot(systemDef) {
  const out = {};
  for (const [res, amount] of Object.entries(systemDef.rewards)) {
    out[res] = Math.max(1, Math.floor(amount * INVADED_LOOT_FRACTION));
  }
  return out;
}
